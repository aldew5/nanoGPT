...
def zeropower_via_svd(G, steps=None):
    """Computes G^0 = UD^0V^T. We call this the orthogonalization of G."""
    U, S, V = G.svd()
    return U @ V.T
...
class Muon(torch.optim.Optimizer):
    ...
    '''
    def __init__(self, params, lr=3e-4, momentum=0.98, nesterov=True, weight_decay=1e-4,
    ...
    def step(self):
        for group in self.param_groups:
            ...
            updates_flat = torch.zeros(total_params, device=self.device, dtype=torch.bfloat16)
            curr_idx = 0
            for i, p in enumerate(group['params']):
                ...
                if p.grad.shape[0]>p.grad.shape[-1]:
                    ...
                    g = p.grad.view(-1, p.grad.shape[-1])
